...
class Node:
    def __init__(self, judul, pengarang=None):  # pengarang dibuat opsional
        self.judul = judul
...
def insertTail(head, judul):
    new_node = Node(judul)  # tetap 1 parameter seperti punyamu
    if not head:
        return new_node
...
# delete by judul (pakai contains biar tetap bisa hapus)
def delete_by_judul(head, judul):
    current = head
    while current:
        if judul in current.judul:  # <-- diperbaiki
            if current.prev:
